...
exports.generate_report = functions.https.onRequest((request, response) => {
  ...
  // we have to convert it to month/ date /year
    ...
    if(startDate == undefined || endDate == undefined){
    	return response.json({success: false, message: "Please provide the date"})
    }
...
try{
   ...
   admin.database().ref('Horses').orderByChild('FEIID').equalTo(feiid).once('value', (snapshot) =>{
     ...
     if(data == null){
      return response.json({success: false,message: "No horse found with this id"})
     }
     ...
     if(horseData.horseData['Admin NF']){
     ...
     if(horseData.Competitions){
      ...
      // here we will competitions in to array
       competitions = horseData.Competitions;
       athelete = competitions.Athlete;
     }else{
        competitions = "----";
        athelete = '----';
     }
     ...
     if(competitionsInRange.length <= 3){
       return response.json({success: false, error: "Not enough data in this date range to compute the report"})
     }
...
competitionsInRange = arr2;
     ...
     competitionsInRange.sort(function(x, y) { return y.ObstHeight - x.ObstHeight; });
       ...
       // console.log(feiid);
         ...
         admin.database().ref(`reports/${feiid}`).update(reportObject, function(error) {
            if (error) {
              ...
              response.json({success:true,data:"data saved successfully"});
            }
         });
